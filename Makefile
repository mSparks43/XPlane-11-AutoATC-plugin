...
# 	-L/home/mSparks/Downloads/jdk1.8.0_40/jre/lib/amd64/server \
# 	-ljvm
# TARGET ?= win.xpl
# all: linktest AutoATC.xpl copy
...
$(TARGET): $(BUILDDIR)/$(TARGET)/64/win.xpl
...
$(BUILDDIR)/obj64/%.o : %.c
	mkdir -p $(dir $@)
	x86_64-w64-mingw32-g++ $(CFLAGS) -m64 -c $< -o $@
	x86_64-w64-mingw32-g++ $(CFLAGS) -MM -MT $@ -o $(@:.o=.cdep) $<

$(BUILDDIR)/obj64/%.o : %.cpp
	mkdir -p $(dir $@)
	x86_64-w64-mingw32-g++ $(CFLAGS) -m64 -c $< -o $@
	x86_64-w64-mingw32-g++ $(CFLAGS) -MM -MT $@ -o $(@:.o=.cppdep) $<
